...
class FixtureNarrativeRepository:
    ...
    def get_status(self) -> MetaStatusResponse:
        ...
        data_as_of = MetaStatusResponse.model_validate(
            {**status_payload, "is_stale": False}
        ).data_as_of
        status_payload["is_stale"] = datetime.now(timezone.utc) - data_as_of > timedelta(hours=1)
