...
### Persisted summary lifecycle (A3)
...
transaction with the run's `run_log` row. It schedules nothing itself: it
takes a `stage_run` context from the caller (stage name `summaries`) and
constructs no provider client. The A3b runner below is its only scheduled
caller.
...
key; the production cadence is A3b's decision (below), not a constant
here. Readers never call the provider; only `ensure_summary` may.
...
### Scheduled summaries (A3b), feature-gated and off by default

`phase0/summary_runner.py` drives the A3 lifecycle from the scheduled
pipeline. `pipeline.py` registers it as the `summaries` downstream
component, after `intelligence`, and builds it **only** when
`PHASE0_SUMMARIES_ENABLED` is `1`/`true`/`yes`/`on` (case-insensitive).
Absent, blank, or `0`/`false`/`no`/`off`, nothing is built, no provider
client is constructed, and no summary table is read — the invocation is
exactly what it was before A3b. Any other value is reported as
`summaries_misconfigured` and does nothing.

It is background work only. Summaries are generated here and nowhere
else; no API request generates one, and the narrative API does not read
them yet (B1).

| | |
|---|---|
| Component | `summaries`, non-mandatory, **not** in `INTELLIGENCE_STAGES`, never called by the coordinator |
| Runs in `--replay` | **never** — replay builds no downstream stage and has no network |
| Provider config | `GEMINI_API_KEY` (required only when enabled), `GEMINI_MODEL`, `GEMINI_MAX_OUTPUT_TOKENS`, `GEMINI_TIMEOUT_MS` |
| Missing key / SDK | `summaries_unconfigured`: zero provider calls, zero summary rows, zero runs |
| Budget | `PHASE0_SUMMARIES_MAX_PROVIDER_CALLS`, non-negative integer, default **20** real provider calls per invocation |
| Retry | `RetryPolicy(cooldown=1h, max_generations=3)` per exact generation key |
| Schema | none added — migration 016 is sufficient |

**Which partitions.** Every ticker holding persisted stories or a theme set
on a day a `themes` run completed within `RETRY_HORIZON` (3 days) of the
repository clock, days ascending and tickers ascending. The sweep reads the
ledger and the persisted rows; it does not share the intelligence
component's selection or its retry episodes, and a summary outcome can
never open, anchor, or extend one. Late evidence re-reconciles a day's
themes, which records a fresh `themes` run, which brings the day back into
the sweep.

**Zero provider calls, and no run, when:**

- the partition's population fails the A2/A3 health gate
  (`summaries.assess_population`) — an M2-only day holds stories and no
  theme set and is counted `refused_theme_set_missing`; every refusal is
  counted by its code;
- a theme's artifact is current under the production policy
  (`summary_lifecycle.current_summary_artifact`) — a cache hit;
- the exact key is suppressed by the retry policy — `cooldown` or
  `exhausted`;
- the budget cannot cover the next theme — `deferred_budget`.

There is no cache of the runner's own: currentness is A3's exact
`(theme_id, input_fingerprint, policy_fingerprint)` key re-validated from a
fresh population. Changed evidence (including theme ids re-minted by a
story change), model, output-token cap, prompt/schema, copy rules or attempt
bound is a new key and regenerates; the old artifact stays in history and is
never current under the new policy.

**One production policy.** `summary_runner.production_generation_policy()`
resolves the policy from the production client's configuration
(`production_summary_client()`, which makes no request when constructed),
`PRODUCTION_MAX_ATTEMPTS` (2) and the copy rules. The scheduler's pre-check
and its generation use the same resolved policy; a later read-only API must
resolve currentness through this same function. The policy carries no
credential.

**Provider-call budget — exact semantics.** Themes are processed in
salience order (`salience_rank`, then `theme_id`) within a partition, and
partitions in day/ticker order. Before a theme starts, the policy's
`max_attempts` is reserved; if less than that remains, the theme and every
later one are deferred (no call, no row) and are ordinary candidates next
invocation. After the call the reservation is settled at the outcome's real
`provider_calls`; a call that raised keeps its whole reservation. Real
provider calls per invocation therefore never exceed the budget.

**Run ownership and failure isolation.** One `summaries` run per ticker-day
(`<invocation>:summaries:<ticker>:<day>`), opened only when some theme needs
a provider call. A typed `unavailable` generation is partial work: it is
recorded (generation and attempts, no artifact), the run ends `degraded`,
and the partition's remaining themes are still processed. An unexpected
exception is different: a logged mutation that raises settles its run
`failed` and the run refuses every later mutation, so the exception ends
that partition (its remaining themes are counted `themes_not_attempted`),
the run is recorded `failed`, and the sweep continues with the next
partition. Whatever committed earlier stays durable. Nothing in this
component writes evidence, stories or themes, so no summary outcome can
remove them.

**Status.** Counters include `partitions_considered`, `partitions_refused`
(plus `refused_<code>`), `partitions_current`, `partitions_with_work`,
`partitions_settled`, `partitions_failed`, `themes_considered`, `cache_hits`,
`generated`, `unavailable`, `cooldown`, `exhausted`, `deferred_budget`,
`themes_not_attempted`, `provider_calls` and `provider_call_budget`.
Unavailable themes, budget deferral, retry suppression and partition
failures are reported as errors, so the component is `degraded` (or
`failed` when it settled nothing, e.g. unconfigured) and the invocation is
`degraded` at worst — never `failed` on account of summaries. Errors pass
through the shared redaction boundary.

**The configured key is never persisted.** `redact_secrets` removes a
credential a name introduces (`api_key=...`). The configured
`GEMINI_API_KEY` value is also removed wherever it appears bare, at the
source: the runner wraps the provider client so a provider error echoing
the key is re-raised as the *same* error class with the key replaced before
A2 records the attempt (timeouts stay timeouts, authentication failures
stay configuration failures, retries are unchanged), and an unexpected
exception carrying it is re-raised scrubbed before `stage_run` writes
`run_log.errors`. Returned component errors are scrubbed as well.

**An execution identity is used once.** If a `summaries` run with a
partition's run id already exists — a caller reused an invocation id — no
provider work is done under it; the partition is reported as
`summary_run_identity_reused` (`partitions_identity_reused`) and the
existing run row is left exactly as it was. A fresh invocation id retries,
...
| Summarization | **live only** — the feature-gated A3b `summaries` component; replay never builds it and never calls a provider |
