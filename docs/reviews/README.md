...
not ratified a review protocol, and no summaries have been generated by the
scheduled path yet (A3 persists them — `summary_artifacts` and its sentence
and citation tables — and A3b can schedule generation, but only behind
`PHASE0_SUMMARIES_ENABLED`, which is off by default). The tooling is
